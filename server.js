...
// ═══════════════════════════════════════════════════
// ── VIRAL HOOKS — embedded from viral_hooks.csv (1003 hooks) ────────────
...
const USED_HOOK_IDS = new Set(); // tracks used hook IDs to enforce variety across generations
const MAX_USED_HISTORY = 50;     // keep last 50, then start cycling
...
async function selectAndFillHooksViaHaiku(count, context, excludeIds = []) {
  if (VIRAL_HOOKS.length === 0) return [];
...
- VARIETY REQUIRED: Each selected hook must be from a different category (EDU, STO, AUT, MYT, COM, DAY, RAN). Do not select two hooks from the same category.
- EXCLUDE THESE IDs — already used recently, do not select them: \${excludeIds.length ? excludeIds.join(', ') : 'none'}
- Think broadly — portrait sessions, lighting, posing, client experience, gear, before/after session, confidence, the business of photography. Don't default to the same 5 hooks every time.
...
function randomHooks(count, funnelStage, pillar, excludeIds = []) {
  const categoryMap = {
  ...
  const filtered = VIRAL_HOOKS.filter(h => categories.includes(h.category) && !excludeSet.has(h.id));
...
// ── DEBUG: Hook system status + reset ────────────────────────────────────────
app.get('/api/debug/hooks', requireAuth, (req, res) => {
  res.json({
    viralHooksLoaded: VIRAL_HOOKS.length,
    usedHookCount: USED_HOOK_IDS.size,
    usedHookIds: [...USED_HOOK_IDS],
    categories: [...new Set(VIRAL_HOOKS.map(h => h.category))],
    sample: VIRAL_HOOKS.slice(0, 3).map(h => ({ id: h.id, category: h.category })),
  });
});

app.post('/api/debug/hooks/reset', requireAuth, (req, res) => {
  USED_HOOK_IDS.clear();
  res.json({ cleared: true, message: 'Used hook history reset' });
});
...
2. script — what Ian says. 4-8 sentences. Moves forward, no wrap-up. Write like the RIGHT examples in system prompt.
...
Do NOT include a hook field — Node attaches it.
...
app.post('/api/content/ideate', requireAuth, async (req, res) => {
  try {
    ...
    let selectedHooks = [];
    const excludeIds = [...USED_HOOK_IDS];
    try {
      selectedHooks = await selectAndFillHooksViaHaiku(count, hookContext, excludeIds);
    } catch (e) {
      console.error('Hook selection failed, using random fallback:', e.message);
    }
    if (selectedHooks.length < count) {
      const usedSet = new Set(selectedHooks.map(h => h.id));
      const fallback = randomHooks(count - selectedHooks.length, funnel_stage || 'ToFu', pillar || '', [...excludeIds, ...usedSet]);
      selectedHooks = [...selectedHooks, ...fallback].slice(0, count);
    }
    const lockedHooks = formatLockedHooks(selectedHooks);
...
3. funnelStage
4. lane
5. script — what Ian actually says. 4-8 sentences. Write like the RIGHT examples in the system prompt. Moves forward, no wrap-up.
...
Return as a JSON array only. No preamble, no markdown fences.`;
    ...
    // Track used hook IDs to enforce variety on next generation
    selectedHooks.forEach(h => {
      USED_HOOK_IDS.add(h.id);
      if (USED_HOOK_IDS.size > MAX_USED_HISTORY) {
        USED_HOOK_IDS.delete(USED_HOOK_IDS.values().next().value);
      }
    });

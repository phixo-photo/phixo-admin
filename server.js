...
// ═══════════════════════════════════════════════════
// ── VIRAL HOOKS — 1003 hooks embedded ────────────────────────────────────
...
// Track used IDs so Haiku can't repeat the same hooks across generations
...
async function selectAndFillHooksViaHaiku(count, context) {
  ...
  // ── DIVERSITY: shuffle hooks and send a random balanced slice ──
  // Sending all 1003 hooks causes Haiku to always pick the same "best" ones
  // at the top of each category. Instead: shuffle within each category,
  // take 20 per category, then shuffle again. Haiku physically cannot
  // pick the same hooks twice because it never sees the same slice twice.
  const categories = ['EDUCATIONAL', 'STORYTELLING', 'MYTH BUSTING', 'AUTHORITY', 'COMPARISON', 'DAY IN THE LIFE', 'RANDOM'];
  const PER_CAT = 20;

  // Also exclude recently used IDs
  const usedIds = new Set(USED_HOOK_IDS);

  let slice = [];
  for (const cat of categories) {
    const pool = VIRAL_HOOKS
      .filter(h => h.category === cat && !usedIds.has(h.id))
      .sort(() => Math.random() - 0.5)
      .slice(0, PER_CAT);
    slice.push(...pool);
  }
  // Final shuffle so Haiku doesn't see categories grouped
...
3. Each selected hook MUST be from a DIFFERENT category. No two hooks can share a category.
4. For each selected hook, fill in all placeholder text (anything in parentheses) with specific portrait photography context.

RULES:
- Return ONLY a JSON array. No explanation, no preamble, no markdown.
- Every "id" MUST exist exactly in the database below — copy character for character.
- Do not invent, modify, or combine IDs.
- "filledHook" = the template with ALL placeholders replaced. Keep the sentence structure.
- DIFFERENT CATEGORIES REQUIRED — no two can share a category.

RETURN FORMAT:
[
  {"id": "EDU_0047", "filledHook": "If you just booked your first headshot session and you don't want to look stiff, here are 3 things to do immediately."},
  {"id": "STO_0012", "filledHook": "..."}
]
...
${csvText}`;
  ...
  try {
    ...
    console.error('Haiku hook selection error:', e.message);
    return [];
  }
  ...
  console.log(`Hook selection: slice=${slice.length}, Haiku returned ${returned.length}, ${validated.length} validated`);
  return validated;
}
...
Return as a JSON array only. No preamble, no markdown fences.`;
    ...
    // Update used-hook tracking so next generation can't repeat these
